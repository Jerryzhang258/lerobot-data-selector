...
class DataSelector:
    ...
    def _build_ctrl_bar(self) -> np.ndarray:
        ...
        for spd in speeds:
            ...
            rect   = (bx, by, spw, bh)
            draw_button(bar, rect, f"{spd}x", bg, border_color=border, scale=0.52)
            self._btn_speeds.append((rect, spd))
            bx += spw + 4

        # ── 键盘提示（底部）
        hints = "A / D : frame    W / S : episode    Space : play    X : delete    U : undo    Q : quit"
    ...
    def _on_mouse(self, event, x, y, flags, param):
        ...
        y_offset += self.CAM_H * self.CAM_ROWS + self.SEP * (self.CAM_ROWS - 1) + self.SEP
    ...
    def run(self):
        ...
        while True:
            ...
            if self.playing:
                ...
                step_t = 1.0 / (fps_data * self.play_spd)
                if dt >= step_t:
            ...
            wait_ms = 15 if self.playing else 30
            ...
            elif key in (ord('x'), ord('X'), 255):   # X or Delete
                self._mark('delete')
    ...
    def _save_results(self):
        ...
        unrev   = sorted(ep for ep in range(self.n_ep)
        ...
        result = {
            ...
            "unreviewed": {"count": len(unrev),   "episodes": unrev},
        }
        ...
        print(f"  Unreviewed:  {len(unrev)} episodes")
        print(f"{'='*60}\n")
